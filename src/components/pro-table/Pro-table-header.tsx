...
export default memo(function (props: HeaderProps) {
    ...
    const [colSetting] = useState({ ...colSettingDefault, ...tabs?.secondTabs?.col }) // 列配置
        ...
        <header className="pro-table-header-wrap">
            ...
            }
            {
                ...
                && <section className="first">
                    <Tabs defaultActiveKey={tabs.firstTabs.defaultKey} onChange={(e) => tabFirstTabsChange(tabs.firstTabs.key, e)} >
                        {tabs.firstTabs.data.map(item => <TabPane tab={item.label} key={item.key} />)}
                    </Tabs>
                </section>
            }
                ...
                && <section className="second">
                    ...
                    <Row className="second-content" gutter={[0, 16]}>
                        {
                            tabs.secondTabs.data.map((item, index) => {
                                if (rotate) {
                                    return (<Col key={item.key} {...colSetting}>
                                        <Button
                    ...
                    <div className="toggle-wrap">
                        ...
                        <div className="toggle-item">
                            <Button type="link" onClick={() => rotate ? setRotate(0) : setRotate(0.5)}>{rotate ? '收起' : '展开'}<DownOutlined style={{ "transition": "all 0.3s ease 0s", "transform": `rotate(${rotate}turn)` }} /></Button>
                        </div>
                    </div>
